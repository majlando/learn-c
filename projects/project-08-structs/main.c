...
/* Extend with a nested struct example */
...
/* small function to print Node */
static void print_node(const struct Node *n) {
    printf("Node id=%d pos=(%d,%d)\n", n->id, n->pos.x, n->pos.y);
}

/* Example usage in an alternate function to keep main deterministic above */
int example_node(void) {
